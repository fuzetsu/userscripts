  ...
  if (settings.isListPage || settings.isDetailPage) {
    ...
    submit(function (next) {
      try {
        ...
        // 缓存过期时间（小时）
        .eq(2).spinner({ min: 0, spin: function spin(event, ui) {
        ...
        //系统帮助
        .eq(5).button({ icon: 'ui-icon-help', showLabel: false }).prop('href', 'https://github.com/niubilityfrontend/userscripts/tree/master/findingteacheron51talk').prop('target', '_blank').end();
        $('#buttons1>button')
        //反选时间段
