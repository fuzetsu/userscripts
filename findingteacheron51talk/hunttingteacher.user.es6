// ==UserScript==
// @name         51talk选择最好最合适的老师-经验-好评率-年龄-收藏数-Vuejs
// @version      2020.4.3001
// @namespace    https://github.com/niubilityfrontend
// @description  辅助选老师-排序显示，经验值计算|好评率|显示年龄|列表显示所有教师
// @author       jimbo
// @license      OSL-3.0
// @supportURL   https://github.com/niubilityfrontend/findingteacheron51talk
...
.s-t-page {   padding-top: 2px;}
  ...
  function updateTeacherinfoToUI(jqel, tinfo) {
    ...
    jqel
      .find(".teacher-name")
      .html(
        jqel.find(".teacher-name").text() +
          "<br />[" +
          tinfo.label +
          "|" +
          tinfo.thumbupRate +
          "%|" +
          tinfo.favoritesCount +
          "]"
      );
    jqel
      .find(".teacher-age")
      .html(
        jqel.find(".teacher-age").text() +
          " | <label title='排序指标'>" +
          tinfo.indicator +
          "</label>"
      );
  ...
  function executeFilters(uifilters) {
    ...
    $.each($(".item"), function(i, item) {
      ...
      if (
        tinfo.thumbupRate >= uifilters.rate1 &&
        tinfo.thumbupRate <= uifilters.rate2 &&
        tinfo.label >= uifilters.l1 &&
        tinfo.label <= uifilters.l2 &&
        tinfo.age >= uifilters.age1 &&
        tinfo.age <= uifilters.age2 &&
        tinfo.favoritesCount >= uifilters.fc1 &&
        tinfo.favoritesCount <= uifilters.fc2
      ) {
        if (node.is(":hidden")) {
...
正在获取当前时段第${curPageId}页，剩余${remainPages}页总计${totalPages}页,约${remainPages *
          28}个教师<br>
总计选择${sessionStorage.getItem(
          "selectedTimeSlotsTotal"
        )}个时段，未获取${sessionStorage.getItem(
          "selectedTimeSlotsRemain"
        )}个时段，
</p>
</div>`);
          ...
          $.ajax({
            ...
            success: function(r) {
              ...
              if (jqr.find(".teacher-name-tit").length > 0) {
                ...
                tempitem.innerHTML = tempitem.innerHTML
                  .replace("<!--", "")
                  .replace("-->", "");
              }
              if (jqr.find(".evaluate-content-left span").length >= 3) {
                ...
                let thumbupRate =
                  ((thumbup + 0.00001) / (thumbdown + thumbup)).toFixed(2) *
                  100;
                ...
                let isfavorite =
                  jqr.find(".go-search.cancel-collection").length > 0;
                let tage = Number(
                  jqr
                ...
                let tinfo = {
                  ...
                  indicator:
                    Math.ceil((teacherlistinfo.label * thumbupRate) / 100) +
                    favoritesCount,
                ...
                console.log(
                  "Teacher s detail info getting error:" +
                    JSON.stringify(jqel) +
                    ",error info:" +
                    r
                );
  ...
  if (settings.isDetailPage) {
    function processTeacherDetailPage(jqr) {
      ...
      if (window.location.href.toLocaleLowerCase().includes("teachercomment")) {
        ...
        tinfo.thumbupRate =
          (
            (tinfo.thumbup + 0.00001) /
            (tinfo.thumbdown + tinfo.thumbup)
          ).toFixed(2) * 100;
        tinfo.indicator =
          Math.ceil((tinfo.label * tinfo.thumbupRate) / 100) +
          tinfo.favoritesCount;
      ...
      tinfo.tage = Number(
        jqr
          .find(".teacher-name-tit > .age.age-line:eq(1)")
          .text()
          .match(num)
          .clean("")[0]
      );
  ...
  if (settings.isListPage || settings.isDetailPage) {
    ...
    submit(function(next) {
      try {
        ...
        $("body").append(
          "<div id='teachlistdialog' style='display:none;'></div>"
        );
        ...
        $("#buttons>button,#buttons>input,#buttons>a")
          //升序
          .eq(0)
          .button({ icon: "ui-icon-arrowthick-1-n", showLabel: false })
          .click(function() {
            $("#desc").show();
            $(this).hide();
            sortByIndicator(asc);
          ...
          //清空缓存
          .eq(3)
          .button({ icon: "uiicon-trash", showLabel: false })
          .click(function() {
          ...
          //submit suggestion
          .eq(4)
          .button({ icon: "ui-icon-comment", showLabel: false })
          .prop(
            "href",
          ...
          .prop(
            "href",
            "https://github.com/niubilityfrontend/userscripts/tree/master/findingteacheron51talk"
          )
          .prop("target", "_blank")
          .end();
        $("#buttons1>button")
          ...
          .end();
        //初始化时间选择按钮
        $("div.condition-type:eq(0)>ul.condition-type-time>li").each(function(
          i,
          item
        ) {
        ...
        if (timesstr) {
          ...
          if (selectedTimeSlots.length > 0) {
            ...
            while (i--) {
              $(
                "#timesmutipulecheck>input[value='" +
                  selectedTimeSlots[i] +
                  "']"
              ).attr("checked", true);
            }
        ...
        $("#_thumbupRate").html(
          uifilters.rate1 + "% - " + uifilters.rate2 + "%"
        );
      } catch (ex) {
        console.log(ex + "");
      } finally {
