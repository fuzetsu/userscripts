  ...
  if (settings.isListPage || settings.isDetailPage) {
    ...
    submit(function(next) {
      try {
        ...
        css({width: '45px'}).val(GM_getValue('tinfoexprhours', configExprMilliseconds / 3600000)).end().eq(3).button({icon: 'ui-icon-trash', showLabel: false}). //清空缓存
        click(function() {
        ...
        prop('href', 'https://github.com/niubilityfrontend/userscripts/issues/new?assignees=&labels=&template=feature_request.md&title=').prop('target', '_blank').end().eq(5).button({icon: 'ui-icon-help', showLabel: false}). //系统帮助
        prop('href', 'https://github.com/niubilityfrontend/userscripts/tree/master/hunttingteacheron51talk').prop('target', '_blank').end();

        $('#buttons1>button').eq(0).button({icon: 'ui-icon-seek-next', showLabel: true}). //submit suggestion
        click(function() {
          var times = [];
