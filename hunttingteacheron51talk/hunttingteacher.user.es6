...
剩余${sessionStorage.getItem(
          "selectedTimeSlotsRemain"
        )}/${sessionStorage.getItem("selectedTimeSlotsTotal")}个时段，<br><br>
当前时段约${totalPages *
          28}个教师，获取第${curPageId}/${totalPages}页，进度${Math.floor(
  ...
  if(settings.isDetailPage) {
    function processTeacherDetailPage(jqr) {
      ...
      if(window.location.href.toLocaleLowerCase().includes("teachercomment")) {
        ...
        tinfo.thumbupRate = (
          (tinfo.thumbup + 0.00001) / (tinfo.thumbdown + tinfo.thumbup)).toFixed(2) * 100;
